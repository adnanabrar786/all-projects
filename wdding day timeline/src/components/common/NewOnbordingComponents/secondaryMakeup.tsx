...
const SecondaryHairMakeup = ({ nextClick }) => {
  const [LocationValue, setValue] = useState<google.maps.places.PlaceResult>();
  const [isLoading, setIsLoading] = useState(false);

  useEffect(() => {
    if (LocationValue) {
      localStorage.setItem(NEW_ONBOARDING_STEPS_KEYS.SECONDARY_HAIR_AND_MAKEUP, JSON.stringify(LocationValue));
  ...
  useEffect(() => {
    const locate = localStorage.getItem(NEW_ONBOARDING_STEPS_KEYS.SECONDARY_HAIR_AND_MAKEUP);
  ...
  return (
    ...
    >
      <Stack>
        <TitleName title="Where will your fiancée do hair & makeup?" />
...
export default SecondaryHairMakeup;
