...
const PrimaryGettingDressed = ({ nextClick }) => {
  const [LocationValue, setValue] = useState<google.maps.places.PlaceResult>();
  const [isLoading, setIsLoading] = useState(false);

  useEffect(() => {
    if (LocationValue) {
      localStorage.setItem(NEW_ONBOARDING_STEPS_KEYS.PRIMARY_GETTING_DRESSED, JSON.stringify(LocationValue));
  ...
  useEffect(() => {
    const locate = localStorage.getItem(NEW_ONBOARDING_STEPS_KEYS.PRIMARY_GETTING_DRESSED);
  ...
  return (
    ...
    >
      <Stack>
        ...
        >
          <TitleName title="Where will you get dressed?" />
          <Typography
            sx={{
              lineHeight: '1.056rem',
              fontSize: '0.875rem',
...
export default PrimaryGettingDressed;
