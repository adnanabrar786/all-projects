...
import { ValidateVendorForm } from 'validators/vendors';
...
interface Props {
  deleteBox?: boolean;
  vendor_types: Category[];
  handleCancel?: () => void;
  setShowEvent?: (val: boolean) => void;
...
const VendorCardForm = ({
  ...
  const formik = useFormik({
    initialValues: {
      ...
      roles: vendor ? [...vendor.roles] : [],
      email: vendor ? vendor.email : '',
      address: vendor ? vendor.address : '',
    },
    validationSchema: ValidateVendorForm(),
      ...
      try {
        setTimeout(async () => {
          await onSubmit({
            ...
            pictureName: selectedImage ? name : vendor?.picture,
          });
        }, 500);
      } finally {
        setTimeout(() => {
  ...
  const setRoles = (props: { id: string; value: string }) => {
    ...
    if (!exist) {
      ...
      formik.setFieldValue('roles', [
        ...formik.values.roles,
        { id: Number(props.id), label: props.value, type: CategoryType.VENDOR },
      ]);
      return;
    }
  ...
  const onSubmitUpload = async (file: File) => {
    try {
      if (file) {
        ...
        await submitImageGuest(file, formik.values.name, name, 'profile');
  ...
  return (
    <form onSubmit={formik.handleSubmit}>
      ...
      >
        <Stack sx={{ gap: '4px' }}>
          <CustomText
            text={vendor ? 'Change Photo' : 'Add photo'}
            sx={{
              fontSize: '12px',
              lineHeight: '18px',
              fontWeight: '600',
          ...
          >
            {selectedImage || (vendor && vendor.picture) ? (
              <Stack
                sx={{
                  width: '100%',
                }}
                ...
                >
                  <ImagePickerContainer>
                    <DropZoneContainer>
                      <Image
                        alt="selected"
                        src={selectedImage ? URL.createObjectURL(selectedImage) : vendor!.picture}
                        fill
                        quality={100}
                      />
                    </DropZoneContainer>
        ...
        >
          ...
          >
            ...
            <Stack
              sx={{
                width: { xs: '300px', lg: '49%', gap: '0.25rem' },
                marginTop: '1rem',
              }}
            >
              <CustomText
              ...
              <MultiSelectDropdown
                ...
                list={vendor_types.map((i) => ({ ...i, id: String(i.id) }))}
...
export default memo(VendorCardForm);
