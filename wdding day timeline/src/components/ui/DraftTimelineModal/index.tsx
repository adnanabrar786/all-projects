...
const DraftTimelineModal = ({ open, text, setText, handleClose, ShareButtonClick }: TModal) => {
  return (
    <>
      <AppModal
        open={open}
        onClose={handleClose}
        className="absolute rounded-3xl top-[50%] left-[50%] w-[90%] sm:w-[70%] md:w-[50%] lg:w-[40%] -translate-x-1/2 -translate-y-1/2 block pb-20 overflow-auto bg-white outline-none overflow-y-auto h-[80%] sm:h-[85%]"
      >
        {/* Modal Content */}
        <div>
          <div className="flex flex-col items-center justify-center mx-6">
            <p className="text-xl xl:text-w_5xl text-center text-purple font-[500]">{AppText.SHARE_DRAFT}</p>
          </div>
          <div className="flex flex-col items-center justify-center m-6 mt-1">
            <p className="text-sm xl:text-w_2xl text-center text-purple">{AppText.EMAIL_A_DRAFT}</p>
          </div>
          ...
          <div className="border-2 border-gray-200 rounded-xl mt-1 m-6 max-h-[500px] text-purple p-4">
            <div className="font-medium ">
              <div className="text-sm font-medium text-gray-400 sm:text-base">
                <p className="mb-2 mt-4 italic font-[500] text-[12px] xl:text-[16px]">[{AppText.EDITABLE_TEMPLATE}]</p>
              </div>
              <div className="mb-3 text-[12px] xl:text-[16px]">
                <TextareaAutosize
                  id="message"
                  name="message"
                  minRows={5}
          ...
          <div className="grid grid-cols-1 gap-2 px-8 pb-1 mt-8 sm:grid-cols-2">
            <div
              className="text-[14px] xl:text-[16px] w-full border-2 order-2 sm:order-1 rounded-full h-12 text-secondary cursor-pointer  bg-white flex justify-center items-center font-medium "
              onClick={handleClose}
            >
              {ButtonText.CANCEL}
            </div>
            <div
              className="text-[14px] xl:text-[16px] w-full sm:ml-2 border-2 order-1 sm:order-2  rounded-full h-12 flex items-center justify-center font-medium text-white cursor-pointer  bg-secondary"
...
export default DraftTimelineModal;
